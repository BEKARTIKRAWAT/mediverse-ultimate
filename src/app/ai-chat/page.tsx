  ...
  const startListening = () => {
    ...
    recognition.onerror = (err: any) => {
      ...
      if (err.error === "language-not-supported") {
        const engRecognition = new (window as any).webkitSpeechRecognition();
        currentRecognition = engRecognition;
        engRecognition.continuous = false;
        engRecognition.interimResults = false;
        engRecognition.lang = "en-US";
        engRecognition.onstart = () => setIsListening(true);
        engRecognition.onend = () => { setIsListening(false); if (currentRecognition === engRecognition) currentRecognition = null; };
        engRecognition.onresult = (e: any) => { setInput(e.results[0][0].transcript); setIsListening(false); };
        engRecognition.onerror = () => { setIsListening(false); currentRecognition = null; };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50">
      <div className="max-w-4xl mx-auto p-4 py-8">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500">English | हिंदी | Hinglish – Auto‑detect language</p>
          <p className="text-xs text-amber-600 mt-1">⏱️ Rate limit: {RATE_LIMIT_MS/1000} seconds between messages (free tier)</p>
        </div>
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 px-4 py-3 border-b flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <Bot className="w-6 h-6 text-blue-500" /><span className="font-semibold">Mediverse AI</span>
          ...
          <div className="border-t bg-white p-4">
            ...
            <p className="text-xs text-gray-400 text-center mt-3">🌐 Auto language detection. Rate limit: {RATE_LIMIT_MS/1000}s between messages (free tier).</p>
          </div>
        </div>
      </div>
    </div>
