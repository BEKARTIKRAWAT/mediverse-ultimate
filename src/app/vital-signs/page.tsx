...
export default function VitalSignsPage() {
  ...
  const chartData = vitals.filter(v => v.type === "heartRate").sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map(v => ({ date: new Date(v.date).toLocaleDateString(), value: v.value }));
  const latestHR = vitals.filter(v => v.type === "heartRate").sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
